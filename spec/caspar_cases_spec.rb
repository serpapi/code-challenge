...
RSpec.describe 'Runner results for Caspar carousel' do
  ...
  let(:output_path) do
    ...
    _output_path = File.join(project_root, 'caspar-output.json')
  end

  let(:input_file_url) do
    spec_root = File.expand_path(__dir__)
    file_path = File.join(spec_root, 'fixtures/pages/caspar.html')
  ...
  let(:expected_result) do
    spec_root = File.expand_path(__dir__)
    file_path = File.join(spec_root, 'fixtures/expected/caspar-results.json')
