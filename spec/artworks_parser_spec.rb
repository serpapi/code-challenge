...
require './lib/artworks_parser'

describe do
  before :all do
    body = File.read('./files/van-gogh-paintings.html')
    @artworks_parser = ArtworksParser.parse(body)
  end

  it 'should have 51 artworks' do
    expect(@artworks_parser.artworks.size).to eq(51)
  end

  context 'artworks' do
    before do
      @artwork = @artworks_parser.artworks.first
    end
    it 'should have a name' do
      expect(@artwork).to respond_to(:name)
    end
    it 'should have a link' do
      expect(@artwork).to respond_to(:link)
    end
    it 'should have extensions' do
      expect(@artwork).to respond_to(:extensions)
      expect(@artwork.extensions).to be_an(Array)
    end
    it 'should have an image' do
      expect(@artwork).to respond_to(:image)
    end
  end
  ...
  context 'first artwork' do
    ...
    it "name should be 'The Starry Night'" do
      expect(@artwork.name).to eq('The Starry Night')
    end

    it "extensions should be ['1889']" do
      expect(@artwork.extensions).to eq(['1889'])
    end

    it 'link should match' do
      expect(@artwork.link).to eq('https://www.google.com/search?gl=us&hl=en&q=The+Starry+Night&stick=H4sIAAAAAAAAAONgFuLQz9U3MI_PNVLiBLFMzC3jC7WUspOt9Msyi0sTc-ITi0qQmJnFJVbl-UXZxY8YI7kFXv64JywVMGnNyWuMXlxEaBJS4WJzzSvJLKkUkuLikYLbrcEgxcUF5_EsYhUIyUhVCC5JLCqqVPDLTM8oAQDmNFnDqgAAAA&npsic=0&sa=X&ved=0ahUKEwiL2_Hon4_hAhXNZt4KHTOAACwQ-BYILw')
    end

    it 'image should match' do
  ...
  context '51st artwork' do
    before do
      @artwork = @artworks_parser.artworks.last
    end

    it "name should be 'Portrait of Adeline Ravoux'" do
      expect(@artwork.name).to eq('Portrait of Adeline Ravoux')
    end

    it "extensions should be ['1890']" do
      expect(@artwork.extensions).to eq(['1890'])
    end

    it 'link should match' do
      expect(@artwork.link).to eq('https://www.google.com/search?gl=us&hl=en&q=Portrait+of+Adeline+Ravoux&stick=H4sIAAAAAAAAAONgFuLQz9U3MI_PNVLi1k_XNzQyyjYzTq7SUspOttIvyywuTcyJTywqQWJmFpdYlecXZRc_YozkFnj5456wVMCkNSevMXpxEaFJSIWLzTWvJLOkUkiKi0cKbrsGgxQXF5zHs4hVKiC_qKQoMbNEIT9NwTElNSczL1UhKLEsv7QCAOSAHzG2AAAA&npsic=0&sa=X&ved=0ahUKEwiL2_Hon4_hAhXNZt4KHTOAACwQ-BYIxQE')
    end
