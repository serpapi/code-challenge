...
RSpec.describe KnowledgePanelCarouselParser do
  let(:parser) { KnowledgePanelCarouselParser.new }
    ...
    context 'when carousel is not present' do
      ...
      context 'when selected knowledge panel tab is overview (muppets)' do
        ...
        it 'raises a parse error (overview has no carousel)' do
          expect { parser.parse(html) }.to raise_error(KnowledgePanelCarouselParser::ParseError, 'Knowledge Panel overview has no carousel')
        end
      end
      ...
      context 'when selected knowledge panel tab is not a carousel (star trek tng)' do
        ...
        it 'raises a parse error (no carousel found)' do
          expect { parser.parse(html) }.to raise_error(KnowledgePanelCarouselParser::ParseError, 'No carousel found in Knowledge Panel')
        end
      end
      ...
      context 'when knowledge panel is not present' do
        ...
        it 'raises a parse error (no knowledge panel found)' do
          expect { parser.parse(html) }.to raise_error(KnowledgePanelCarouselParser::ParseError, 'No Knowledge Panel was found')
        end
      end
    end
  end
