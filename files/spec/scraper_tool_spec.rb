...
describe ScraperTool do
  ...
  describe 'Greenday' do
    before :all do
      root_dir = File.expand_path('.')
      url = "#{root_dir}/files/expected-greenday.json"
      file = File.read(url)
      @expected_result = JSON.parse(file)

      greenday_url = "#{root_dir}/files/greenday.html"
      tool = ScraperTool.new(greenday_url)
      @result_hash = JSON.parse(tool.scrape_carousel)
    end

    it 'has a keyword' do
      expect(@result_hash.keys).to eq(['members'])
    end
  ...
  describe 'Miyazaki' do
    before :all do
      root_dir = File.expand_path('.')
      url = "#{root_dir}/files/expected-miyazaki.json"
      file = File.read(url)
      @expected_result = JSON.parse(file)

      miyazaki_url = "#{root_dir}/files/miyazaki.html"
      tool = ScraperTool.new(miyazaki_url)
      @result_hash = JSON.parse(tool.scrape_carousel)
    end

    it 'has a keyword' do
      expect(@result_hash.keys).to eq(['movies'])
    end
