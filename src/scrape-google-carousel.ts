...
export class GoogleCarouselScraper {
  ...
  /**
   * Google relies on JavaScript to replace image sources with the actual
   * images after the page loads. i.e. the initial image source in the HTML
   * is inaccurate.
   *
   * This method searches the JavaScript to extract out the corresponding
   * image URIs.
   *
   * Preconditions:
   * - JavaScript is defined after the definition of the carousel
   * - Within JavaScript, image URI (i.e. data:image...) is defined before the image ID
  ...
  private getImage(element: cheerio.Element): string | null {
    ...
    // The target image URI can be found before the image id is defined
