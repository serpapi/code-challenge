...
import { parseHtml, parseScriptForImages } from "./extract-carosel-from-html";

import expectedOutput from "../files/expected-array.json";

describe("extract carosel from html", () => {
  test("should parse van gogh html to expected output", async () => {
    const htmlFile = (
...
describe("parse script for images", () => {
  test("should parse script for image data - pretty", () => {
    const script = `
    (function () {
      ...
      var ii = ["kximg0"];_setImagesSrc(ii, s);
    })();
    `;
  ...
  test("should parse script for image data - minified", async () => {
    const script = (
      await readFile(path.resolve("files/script.txt"))
    ).toString();
